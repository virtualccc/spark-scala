package ch9.EstabilshModel.ItemBasedCommand
...
//    }
    ...
    trainUserItemRating.take(1).foreach(print)

    // 抽取菜品评分数据，取平均分(item, rating(mean))
    val trainItemRating1 = trainData.map{case (user,item,rating)=>(item,rating)}
   trainItemRating1.take(1).foreach(print)

      val trainItemRating= trainItemRating1.groupByKey().map{
